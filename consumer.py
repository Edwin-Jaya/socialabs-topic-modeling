...
from services.etm import EmbeddedTopicModeling
...
def topicModelling(dataGatheringQueue):
    try:
        ...
        for tweet in tweets_cursor:
            ...
            dataForEtm.append(tweet['full_text'])

        data = Preprocessing(dataForEtm, keyword).get_data()
        etm = EmbeddedTopicModeling()
        etm_model = etm.generateTopic()
        topics = etm_model[2]['topics']
        # topics = lda_model.show_topics(log=False, formatted=False)
        documents_prob = etm.document(dataTweet, data, etm_model)
